...
case class PositionRange(from: Int, to: Int)

trait Xml extends Core {
  // WARNING. UGLY HACK!! This parser has been extended to collect positions
  // of scala expressions inside xml literals. To achieve this,
  // we maintain a set of position ranges for scala expressions inside
  // xml literals. Because the set builder is global for each Scala instance,
  // we need a new instance every time we parse something.
  //
  // A more appropriate approach would be to adapt all the parsers to return a
  // a Set[PositionRange], but that would require more changes.
  private val scalaExprPositions = Set.newBuilder[PositionRange]
  def getScalaExprPositions = scalaExprPositions.result()
  protected def addScalaExprPositionRange(range: PositionRange): Unit = {
    scalaExprPositions += range
  }
  ...
  private[this] object Xml{
    ...
    val ScalaExpr: P0 = P(Index ~ "{" ~ WS ~ Block ~ WL ~ "}" ~ Index).map {
      case (from, to) =>
        addScalaExprPositionRange(PositionRange(from, to))
        Unit
    }
    val Char = P( AnyChar )
